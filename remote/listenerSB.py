...
matrix_L = numpy.matrix(

			'25   30   40   50   50    50    52    55    55    57.5    62.5;\
...
# '25   30   40   50   50    50    55    55    60    65    75;\
...
def control_motor(data):
    ...
    elif l == 0 and r == 1 and ticker < 5 and flag == 0:
		flag_t = time.time()+1
		flag = 1
		ticker = ticker + 1

    elif l == 1 and r == 1 and flag == 0:
        flag_t = time.time()
        flag = 1
        state = LOW_POWER_THROTTLE
    ...
    #50us corrections
    if state == LOW_POWER_THROTTLE
    	offsetL = 170
        offsetR = 170
    elif ticker > 0: # Want more right, so add to left motor
		offsetL = ticker * 50
		offsetR = 0
    elif ticker < 0: # Want more left, so dd to right motor
		offsetR = ticker * 50
		offsetL = 0
    elif ticker == 0:
        offsetL = 0
